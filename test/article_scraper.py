...
def get_real_news():
    ...
    df['Date'] = df['Date'].apply(lambda date: datetime.strptime(date, "%a, %d %b %Y %H:%M:%S %Z").strftime("%B %#d, %Y"))
    return df

#The following functions scrape article from prominent fake/satirical news websites
def empire_news_crawler():
    categories = ['entertainment/', 'sports/', 'politics/', 'business/', 'healthfitness/', 'sciencetech/', 'world']
    df = pd.DataFrame(columns=['Title', 'Date', 'Label'])
    for link in categories:
        ...
        for text in header_texts:
            ...
            df_entry = pd.DataFrame({'Title': [title], 'Date': [date], 'Label':['Fake']})
...
def the_onion_crawler():
    ...
    for page in range(1,5):
        ...
        # Find a article by its class name
        headers = soup.find_all('h3', class_='is-style-scale-2 wp-block-post-title has-delta-font-size')
        header_texts = [header.get_text(strip=True) for header in headers]
        for text in header_texts:
            df_entry = pd.DataFrame({'Title': [text], 'Label':['Fake']})
            df = pd.concat([df, df_entry], ignore_index=True)
...
df = pd.concat([df, empire_news_df, now8news_df, the_onion_df, real_news_df], ignore_index=True)
