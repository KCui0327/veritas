...
def find_duplicates(embeddings, sentences, similarity_threshold=0.8, batch_size=1000):
    ...
    for i in range(0, n_samples, batch_size):
        if i % (batch_size * 10) == 0:  # Print progress every 10 batches
            print(
                f"Processing batch {i//batch_size + 1}/{(n_samples + batch_size - 1)//batch_size}"
            )
        ...
        for j in range(len(batch_embeddings)):
            ...
            for similar_idx in similar_indices:
                if similar_idx not in duplicates:
                    ...
                    print(
                        f"  Index {global_idx}: {sentences.iloc[global_idx][:100]}..."
                    )
                    print(
                        f"  Index {similar_idx}: {sentences.iloc[similar_idx][:100]}..."
                    )
...
duplicates = find_duplicates(
    embeddings, sentences, similarity_threshold=0.8, batch_size=500
)
end_time = time.time()
